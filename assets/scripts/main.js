...
function addRecipesToDocument(recipes) {
  ...
  for (const elem of recipes) {
    let currRecipe = document.createElement("recipe-card");
    currRecipe.data = elem;
    ref.append(currRecipe);
  }
}
...
function initFormHandler() {
  ...
  const formDataObj = new FormData(formRef);
  ...
  const getRlist = getRecipesFromStorage();
  const addRecipe = getRlist.push(recipeObject);
  saveRecipesToStorage(getRlist);
